...
def convert(args):
    ...
    hedge_names = ['request_confirmation', 'tag_question', 'empty,down', 'in_group_marker', 'expletives',
                   'competence_mitigation', 'ack']

    import pandas as pd
    print(args.hedge_file)
    df = pd.read_csv(args.hedge_file, names=hedge_names)
    ...
    with open(args.in_file, 'r') as fh:
        ...
        for mr, text, voice in csvread:
            ...
            da = DA.parse_diligent_da_hedge(da,df.iloc[idx] ,hedge_names)
...
if __name__ == '__main__':
    ...
    argp.add_argument('hedge_file', help = "hedge_file_name")
